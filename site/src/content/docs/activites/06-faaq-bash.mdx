...
# Script qui présente l'utilisateur.

nom="Henri"
echo "Bonjour, je m'appelle $nom."
echo "Je suis en train d'apprendre bash."
```
...
# Je suis en train d'apprendre bash.
```
...
dossier="/home/alice/documents"
taille="450 Ko"

echo "Le fichier $nom_fichier est dans $dossier et fait $taille."
```
...
Pour que `adduser` fonctionne dans un script sans te poser de questions, utilise les options `--disabled-password --gecos ""`. Tu les reverras dans la Partie 3 — elles rendent la création **non interactive**.
</Aside>
...
# Création de plusieurs utilisateurs de test via une boucle.

utilisateurs="testuser_a testuser_b"

for nom in $utilisateurs; do
    adduser --disabled-password --gecos "" "$nom"
    echo "→ $nom créé"
done
```
...
sudo ./creer-utilisateurs.sh
```

Vérifie avec `getent passwd testuser_a testuser_b` que les deux comptes existent.

**Nettoyage après test** — supprime les comptes pour libérer les noms :

```bash
sudo deluser --remove-home testuser_a
sudo deluser --remove-home testuser_b
```
