...
Sur ton ordinateur personnel, tu es probablement le seul à l'utiliser. 
Tu ouvres, modifies et supprimes n'importe quel fichier sans te poser de questions. 
Sous Linux, et particulièrement sur un **serveur web**, bénéficier d'autant de liberté 
...
Si `root` peut tout faire, pourquoi ne pas toujours travailler en tant que root? Parce qu'un pouvoir illimité rend chaque erreur catastrophique. Un `rm -rf /` accidentel en tant qu'utilisateur normal ne fait pas grand-chose — le système refuse. En tant que `root`, il efface tout.
...
Les mots de passe ne sont **pas** dans `/etc/passwd` (malgré le nom). Ils sont stockés sous forme de **hash** dans `/etc/shadow`, un fichier lisible uniquement par `root`. Cette séparation est une mesure de sécurité : `/etc/passwd` doit être lisible par tous (les programmes en ont besoin pour associer les UID aux noms), mais les mots de passe doivent rester protégés.
...
sudo chown :webdev fichier.txt          # Change seulement le groupe
...
```bash
sudo chgrp webdev index.js             # Change le groupe du fichier
sudo chgrp -R webdev /var/www/monsite  # Récursif
```

### Cas pratique : configurer un site web

Voici un scénario complet qui combine toutes les commandes. Tu configures les permissions d'un site web pour que l'équipe de développement puisse modifier les fichiers, et que le serveur web puisse les servir.
...
# 4. Permissions : propriétaire et groupe peuvent lire/écrire,
#    les autres (dont www-data) peuvent seulement lire
# Répertoires : 775 (rwxrwxr-x) — traversable par tous
sudo find /var/www/monsite -type d -exec chmod 775 {} \;
# Fichiers : 664 (rw-rw-r--) — modifiables par le groupe
sudo find /var/www/monsite -type f -exec chmod 664 {} \;

# 5. S'assurer que les nouveaux fichiers héritent du groupe
sudo chmod g+s /var/www/monsite

# 6. Ajouter www-data au groupe pour que Nginx puisse lire
...
<Aside type="tip" title="Le bit setgid (g+s)">
Le `chmod g+s` sur un répertoire fait en sorte que les nouveaux fichiers créés à l'intérieur héritent automatiquement du groupe du répertoire (ici `webdev`), plutôt que du groupe principal de l'utilisateur qui les crée. Indispensable pour le travail collaboratif.
</Aside>
